"""
Claude CLI worker for AgentWhip.

Worker that uses the installed 'claude' CLI via subprocess.
"""
...
class ClaudeCLIWorker(Worker):
    ...
    This worker calls the 'claude' command installed on the system.
        ...
        try:
            if not self.is_available():
                return self.create_failure_result(
                    ...
                    output="Claude CLI not available",
                )
    ...
    def _parse_response(self, response: str, task_id: str) -> TaskResult:
        """
        Parse Claude CLI response into TaskResult.
...
class MockClaudeCLIWorker(Worker):
    """
    Mock Claude CLI worker for testing.
    ...
    def __init__(
        ...
        super().__init__(config, project_path, handover_context)
        self.model = config.model or "mock-claude-cli"
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        result = self.create_success_result(
            output=f"[Claude CLI] Mock execution of task: {task.description}\n"
                   f"Task ID: {task.id}\n"
                   f"Phase: {task.phase}\n"
